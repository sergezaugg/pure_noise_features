...
with c1:
    n1 = st.number_input(label = "n1",  min_value=10, max_value=10000, value=1000, step=10,)
with c2:
...
with c6:
    corr1 = st.number_input(label = "corr1", min_value=-1.0, max_value=+1.0, value=0.0, )

c1, c2, c3, c4, c5, c6, = st.columns(6)
with c1:
    n2 = st.number_input(label = "n2",  min_value=10, max_value=10000, value=1000, step=10,)
with c2:
...
with c6:
    corr2 = st.number_input(label = "corr2", min_value=-1.0, max_value=+1.0, value=0.0, )
...
c0, c1, c2,  = st.columns([0.20, 0.35, 0.65])
...
with c2:  
    ...
    with st.form("B", border=False):
        ...
        if submitted:   
            ...
            ss["fig02"].update_layout(yaxis_range=[-0.1, +1.1])
